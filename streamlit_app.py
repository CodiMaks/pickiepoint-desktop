...
try:
    cursor.execute("SELECT youtube_content FROM areas WHERE customer_id = ?", (st.session_state['area_customer_id'], ))
    youtube_text_area_value = cursor.fetchone()[0]
    cursor.execute("SELECT pre_paraphrase_content FROM areas WHERE customer_id = ?", (st.session_state['area_customer_id'], ))
    pre_paraphrase_text_area_value = cursor.fetchone()[0]
    cursor.execute("SELECT paraphrase_content FROM areas WHERE customer_id = ?", (st.session_state['area_customer_id'], ))
    paraphrase_text_area_value = cursor.fetchone()[0]
    cursor.execute("SELECT summary_content FROM areas WHERE customer_id = ?", (st.session_state['area_customer_id'], ))
...
if 'forgot_password_token' not in st.session_state:
    st.session_state['forgot_password_token'] = False
...
if st.session_state.current_page == 'Youtube':
                ...
                if lang_code != lang_code_to_translate_to:
                    ...
                    if lang_code_to_translate_to not in arabic_languages:
                        ...
                        cursor.execute("UPDATE areas SET youtube_content = ? WHERE customer_id = ?",
                                       (translated_text, areas_customer_id))
                        conn.commit()
                        conn.close()
                        ...
                        if lang_code_to_translate_to == "ar" or lang_code_to_translate_to == "fa" or lang_code_to_translate_to == "ur" or lang_code_to_translate_to == "ku" or lang_code_to_translate_to == "he":
                            ...
                            cursor = conn.cursor()
                            cursor.execute("UPDATE areas SET youtube_content = ? WHERE customer_id = ?",
                                           (full_arabic_text, areas_customer_id))
                            conn.commit()
                            conn.close()
                ...
                if lang_code != lang_code_to_translate_to:
                    ...
                    if len(youtube_text_area) > MAX_CHUNK_SIZE:
                        ...
                        if lang_code_to_translate_to not in arabic_languages:
                            ...
                            cursor = conn.cursor()
                            cursor.execute("UPDATE areas SET youtube_content = ? WHERE customer_id = ?",
                                           (full_translation, areas_customer_id))
                            conn.commit()
                            conn.close()
    ...
    def clear_text_area():
        ...
        if youtube_text_area_value != "":
            cursor.execute("UPDATE areas SET youtube_content = ? WHERE customer_id = ?", ("", areas_customer_id))
        ...
        if continue_flag is True:
            ...
            if continue_flag is True:
                ...
                try:
                    ...
                    cursor.execute("UPDATE areas SET youtube_content = ? WHERE customer_id = ?",
                                   (str(full_text), areas_customer_id))
                    conn.commit()
                    conn.close()
...
if st.session_state.current_page == 'Paraphrase':
            ...
            if len(paraphrase_text_area) < 99000:
                ...
                if lang_code != lang_code_to_translate_to:
                    ...
                    if lang_code_to_translate_to not in arabic_languages:
                        ...
                        conn = sqlite3.connect('text_areas.db')
                        cursor = conn.cursor()
                        cursor.execute("UPDATE areas SET paraphrase_content = ? WHERE customer_id = ?",
                                       (translated_text, areas_customer_id))
                        conn.commit()
                        conn.close()
    ...
    def clear_text_area(widget):
        ...
        if widget == "pre_paraphrase":
            ...
            if pre_paraphrase_text_area_value != "":
                cursor.execute("UPDATE areas SET pre_paraphrase_content = ? WHERE customer_id = ?", ("", areas_customer_id))
            ...
            if paraphrase_text_area_value != "":
                cursor.execute("UPDATE areas SET paraphrase_content = ? WHERE customer_id = ?", ("", areas_customer_id))
    ...
    if paraphrase_but:
        ...
        if len(pre_paraphrase_text_area) > 8 and len(pre_paraphrase_text_area) < 8500:
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE areas SET paraphrase_content = ? WHERE customer_id = ?",
                           (response.text, areas_customer_id))
            conn.commit()
            conn.close()
            st.rerun()
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE areas SET paraphrase_content = ? WHERE customer_id = ?",
                           (full_paraphrased_text, areas_customer_id))
            conn.commit()
            conn.close()
            st.rerun()
...
if st.session_state.current_page == "Summary":
    ...
    conn = sqlite3.connect('settings_save.db')
    cursor = conn.cursor()
    cursor.execute("SELECT summary_type FROM settings WHERE customer_id = ?", (st.session_state['area_customer_id'], ))
    index_summary_type = cursor.fetchone()[0]
    cursor.execute("SELECT summary_mode FROM settings WHERE customer_id = ?", (st.session_state['area_customer_id'], ))
    ...
    with summary_sub_col1:
        summary_type = st_btn_select(("Abstractive", "Extractive"), index=index_summary_type, key="unique_btn_select_key_type")
    ...
    def clear_text_area():
        ...
        if summary_text_area_value != "":
            cursor.execute("UPDATE areas SET summary_content = ? WHERE customer_id = ?", ("", areas_customer_id))
    ...
    def summary_from_youtube():
        ...
        if len(str(summary_text_area)) > 52000:
            ...
            if summary_mode == "Bullet points":
                ...
                cursor = conn.cursor()
                cursor.execute("UPDATE areas SET summary_content = ? WHERE customer_id = ?",
                               (complete_summary, areas_customer_id))
                conn.commit()
                conn.close()
                st.rerun()
                ...
                cursor = conn.cursor()
                cursor.execute("UPDATE areas SET summary_content = ? WHERE customer_id = ?",
                               (full_summary, areas_customer_id))
                conn.commit()
                conn.close()
                st.rerun()
            ...
            if summary_mode == "Bullet points":
                ...
                cursor = conn.cursor()
                cursor.execute("UPDATE areas SET summary_content = ? WHERE customer_id = ?",
                               (sentence_list, areas_customer_id))
                conn.commit()
                conn.close()
                st.rerun()
                ...
                cursor = conn.cursor()
                cursor.execute("UPDATE areas SET summary_content = ? WHERE customer_id = ?",
                               (all_sentences, areas_customer_id))
                conn.commit()
                conn.close()
                st.rerun()
        ...
        if lang_code != "en":
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE areas SET summary_content = ? WHERE customer_id = ?",
                           (response.json()["data"]["translations"][0]["translatedText"], areas_customer_id))
            conn.commit()
            conn.close()
            st.rerun()
            ...
            if summary_source == "URL":
                ...
                if summary_type == "Abstractive":
                    ...
                    if summary_mode == "Plain text":
                        ...
                        cursor = conn.cursor()
                        cursor.execute("UPDATE areas SET summary_content = ? WHERE customer_id = ?",
                                       (response.json()["content"] + " ", areas_customer_id))
                        conn.commit()
                        conn.close()
                        st.rerun()
                        ...
                        cursor = conn.cursor()
                        cursor.execute("UPDATE areas SET summary_content = ? WHERE customer_id = ?",
                                       (bulleted_text, areas_customer_id))
                        conn.commit()
                        conn.close()
                        st.rerun()
                ...
                if cursor.fetchone():
                    ...
                    if login_password_placeholder == cursor.fetchone()[0]:
                        ...
                        try:
                            ...
                            if subs.data:
                                latest_subscription = subs.data[0]
                                subscription_status = latest_subscription.status
                                if subscription_status == "active":
                                    ...
                                    st.write("YOU'VE ENTERED A MYSTERIOUS CONDITION")
                                    st.session_state.current_page = "Subscribe"
                                    st.rerun()
        ...
        if not re.match(email_regex, user_email):
            ...
            try:
                ...
                if cursor.fetchone():
                    ...
                    message = Mail(
                        from_email=('support@pickiepoint.com', 'Pickiepoint'),
...
if st.session_state.current_page == "Verification code":
    ...
    if verify_code_but:
        ...
        if int(verification_code_placeholder) == int(correct_verification_code):
...
if st.session_state.current_page == "Password change":
    ...
    if pass_change_but:
        if len(pass_change_placeholder) < 8:
            ...
            pass_change_message_placeholder.success("Your password has been changed succesfully !", icon="✔")
            time.sleep(2)
            pass_change_message_placeholder.empty()
